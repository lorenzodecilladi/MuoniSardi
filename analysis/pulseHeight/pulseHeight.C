...
//Pulse Height for S3 (palettina) + PMT X2020
void plot(){
  ...
  Float_t PHE[ndata] = {   8.,   15.,   30.,   40.,   63.,   80.}; //[mV]

  for(UInt_t i=0; i<ndata; i++){
    sHV[i]  = HV[i]*0.01; //+-1%
  }
  
  TCanvas *canvas = new TCanvas("canvas","S3 + PMT X2020: average pulse height vs HV", 200, 10, 600, 400);
  ...
  graph -> SetTitle("S3 + PMT X2020: average pulse height vs HV");
  graph -> GetXaxis() -> SetTitle("HV (V)");
  graph -> GetYaxis() -> SetTitle("pulse height (mV)");
  graph -> Draw("AP");
  */

  TGraph *graph = new TGraph(ndata, HV, PHE);
  graph -> SetMarkerStyle(20);
  graph -> SetMarkerColor(kBlue);
  graph -> SetMarkerSize(0.7);
  graph -> SetTitle("S3 + PMT X2020: average pulse height vs HV");
  ...
  cout << endl;
  cout << "\\begin{table}[h!] \n \\centering \n \\caption{S3 + PMT X2020: average pulse height (PHE) vs HV} \n \\label{pulseHeight} \n \\begin{tabular}{ccc} \n \\toprule" << endl;
  cout << "\\# & $\\Delta V_{\\textup{SET}}$ (V) & Average PHE (mV) \\\\" << endl;
  cout << "\\midrule" << endl;
  for(UInt_t i=0; i<ndata; i++){
    //    cout << i+1 << " & " << HV[i] << "$\\pm$" << sHV[i] << " & " << PHE[i] << " \\\\" << endl; //CON ERRORI??????
    cout << i+1 << " & " << HV[i] << " & " << PHE[i] << " \\\\" << endl; // SENZA ERRORI
  }
  cout << "\\bottomrule \n \\end{tabular} \n \\end{table}" << endl;
  cout << endl;
